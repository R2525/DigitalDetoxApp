...
export function DashboardScreen({ onNavigateToMission }: DashboardScreenProps) {
  ...
  // Today's cumulative blocked time (seconds)
  const [todayBlockedSeconds, setTodayBlockedSeconds] = useState<number>(0);
  ...
  useEffect(() => {
    const update = () => {
      try {
        ...
        setTodayBlockedSeconds(isNaN(val) ? 0 : val);
      } catch (_e) {
        setTodayBlockedSeconds(0);
      }
    };
    update();
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Card className="p-4 space-y-3">
              ...
              <div>
                <p className="text-sm text-muted-foreground">Blocked Time (Today)</p>
                <p className="text-lg font-medium">{formatHoursMinutes(todayBlockedSeconds)}</p>
              </div>
            </Card>
